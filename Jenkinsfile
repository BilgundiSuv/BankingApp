    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/BilgundiSuv/BankingApp.git'
                //git 'https://github.com/BilgundiSuv/BankingApp.git'
            }
		}
        stage('Maven Build') {
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t suvarnab/bankapp-eta-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag suvarnab/bankapp-eta-app:${BUILD_NUMBER} suvarnab/bankapp-eta-app:latest"
            }
        }
